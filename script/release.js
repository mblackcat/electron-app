...
function prepareAssets () {
  ...
  const zipAssets = [{
    name: 'electron-app-mac.zip',
    path: path.join(outPath, 'Electron App-darwin-x64', 'Electron App.app')
  }, {
    name: 'electron-app-windows.zip',
    path: path.join(outPath, 'Electron App-win32-ia32')
  }, {
    name: 'electron-app-linux.zip',
    path: path.join(outPath, 'Electron App-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'ElectronAppSetup.exe',
      path: path.join(outPath, 'windows-installer', 'ElectronAppSetup.exe')
    }, {
      name: `electron-app-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `electron-app-${version}-full.nupkg`)
    }])
  })
}
